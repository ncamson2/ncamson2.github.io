...
class SavedPostsDisplay extends StatelessWidget {
  // final String categoryType;
  const SavedPostsDisplay({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<Map<String, dynamic>>>(
      /// THERE WAS A BUG HERE CRASHING THIS PAGE SO
      /// I CHANGED THE FUTURE FUCNTION AWAY FROM fetchCurrentUserSavedPostData
      future:
          context.read<CurrentUserPostCubit>().fetchCurrentUserSavedPostData(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          final currentUserSavedPostList = snapshot.data!;
          // posterId should be something in state so you dont haev to keep
          // rewriting code for different parameters
          final currentUserSavedPostDisplayed = currentUserSavedPostList
                  ...
                  builder: (context, state) {
                    ...
                    if (currentUserSavedPostDisplayed.length == 0) {
                      ...
                      return CurrentUserPostCard(
                        image: currentUserSavedPostDisplayed[index]
                            ['productImageUrl'],
                        productName: currentUserSavedPostDisplayed[index]
                            ['productName'],
                        productPrice: currentUserSavedPostDisplayed[index]
                            ['maximumPrice'],
                        posterId: currentUserSavedPostDisplayed[index]
                            ['posterId'],
                        onTap: () {
                          //! I dont believe this tap is registering ...  causing null
                          ...
                          print(currentUserSavedPostDisplayed[index]['productId']);
                          ...
                          context.read<CurrentUserPostCubit>().productImageUrl(
                                productImageUrl:
                                    currentUserSavedPostDisplayed[index]
                                        ['productImageUrl'],
                              );
                          ...
                          // CurrentUserPostCubit
                          context.read<CurrentUserPostCubit>().productId(
                              productId: currentUserSavedPostDisplayed[index]
                                  ['productId']);
                          print(context
